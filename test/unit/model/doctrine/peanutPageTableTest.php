<?php
  ...
  $t->ok($page->getType(), '->getType() return true');
  ...
  $t->is($page->getType(), 'singlepage', '->getType() return Type');
  ...
  $t->isnt($page->getType(), 'homepage', '->getType() homepage isnt the good value');
  ...
  $page = Doctrine_Core::getTable('peanutPage')->getWhereId('2', $status = 'draft');
  ...
  $page = Doctrine_Core::getTable('peanutPage')->getWhereAuthor('1');
  
  $t->is($page->getFirst()->getAuthor(), '1', '->getAuthor() return 1');
  $t->is($page->getFirst()->getId(), '2', '->getId() return 2 (orderBy created_at DESC)');
  
  
  $page = Doctrine_Core::getTable('peanutPage')->getWhereAuthor('2');
  
  $t->isnt($page, false, '->getAuthor(\'2\') doesnt exist');
  
  
  /**
   * Test a page object by type homepage.
   *
   * @param  string $type The page's status
   *
   * @return peanutPage
   */
   
  $t = new lime_test(2);
  $t->comment('->getHomepage()');
  $page = Doctrine_Core::getTable('peanutPage')->getHomepage($status = 'draft');
  
  $t->ok($page, true);
  $t->is($page->getType(), 'homepage', '->getType() return homepage');
  
  
  /**
   * Test a page object by type.
   *
   * @param  string $type The page's status
   *
   * @return peanutPage
  ...
  $t = new lime_test(2);
  $t->comment('->getAllByType()');
  $page = Doctrine_Core::getTable('peanutPage')->getAllByType('singlepage');
  
  $t->ok($page, true);
  $t->is($page->getFirst()->getType(), 'singlepage', '->getType() return singlepage');
