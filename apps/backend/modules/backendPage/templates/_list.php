<div class="sf_admin_list">
  ...
  <?php else: ?>
    <table id="main_list" cellspacing="0">
      <thead>
        <tr>
          ...
          <?php include_partial('backendPage/list_th_tabular', array('sort' => $sort)) ?>
        ...
        <tr>
          <th colspan="8">
            <?php if ($pager->haveToPaginate()): ?>
              <?php include_partial('backendPage/pagination', array('pager' => $pager)) ?>
            <?php endif; ?>
   ...
   <tbody>
        <?php foreach ($pager->getResults() as $i => $peanut_page): $odd = fmod(++$i, 2) ? 'odd' : 'even' ?>
          <tr id="node-<?php echo $peanut_page['id'] ?>" class="sf_admin_row <?php echo $odd ?><?php
          // insert hierarchical info
          $node = $peanut_page->getNode();
          if ($node->isValidNode() && $node->hasParent())
          {
            echo " child-of-node-".$node->getParent()->getId();
          }
          ?>">
            <?php include_partial('backendPage/list_td_batch_actions', array('peanut_page' => $peanut_page, 'helper' => $helper)) ?>
            <?php include_partial('backendPage/list_td_tabular', array('peanut_page' => $peanut_page)) ?>
            <?php include_partial('backendPage/list_td_actions', array('peanut_page' => $peanut_page, 'helper' => $helper)) ?>
          </tr>
        <?php endforeach; ?>
     </tbody>
